...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const adminMiddleware = async (req: any, res: any, next: any) => {
    try {
      ...
      // Vérifier si l'utilisateur est admin par son ID
      const adminIds = ["44734189", "44735226", "44737331"];
  ...
  app.get("/api/places/search", async (req, res) => {
    try {
      ...
      // Construire l'URL de recherche Google Places
      ...
      // Filtrer pour les magasins alimentaires en Belgique
      searchUrl += "&region=be&type=grocery_or_supermarket";
  ...
  app.get("/api/places/details", async (req, res) => {
    try {
      ...
      if (data.status !== "OK") {
        ...
        return res.status(500).json({ error: "Google Places Details API error" });
      }
